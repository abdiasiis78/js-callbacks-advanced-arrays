...
// The event director needs both the first and last names of each runner
 //for their running bibs.  Combine both the first and last names into
 // a new array called fullName. 
let fullName = [];
...
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names 
//converted to uppercase because the director BECAME DRUNK WITH POWER.
// Convert each first name into all caps and log the result
let allCaps = [];
runners.map(runner => {
    let name = runner.first_name.toUpperCase();
   allCaps.push(name);
 });
...
// The large shirts won't be available for the event due to an ordering 
//issue.  Get a list of runners with large sized shirts so they can
// choose a different size. Return an array named largeShirts that
// contains information about the runners that have a shirt size 
//of L and log the result
...
// Add up all the donations into a ticketPriceTotal variable and log 
//the result
let ticketPriceTotal = runners.reduce((total, runner) => total +
 runner.donation, 0);
...
// data set and the 5k fun run theme.  Try to create and then solve 3
